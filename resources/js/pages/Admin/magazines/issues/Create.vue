    ...
    <TableLayout>
        <template #header>
            <div>
                ...
                <AppText class="mt-1">
                    Create a new issue for {{ magazine.title }}.
                </AppText>
            </div>
        </template>

        <div class="max-w-4xl">
            <Form :action="`/admin/magazines/${magazine.id}/issues`" method="post" class="space-y-6"
                #default="{ errors, processing }">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <AppFormControl label="Title" :error="errors.title" required>
                        <AppInput name="title" placeholder="Enter issue title" />
                    </AppFormControl>

                    <AppFormControl label="Slug" :error="errors.slug">
                        <AppInput name="slug" placeholder="issue-slug" />
                    </AppFormControl>
                </div>

                <AppFormControl label="Subtitle" :error="errors.subtitle">
                    <AppInput name="subtitle" placeholder="Enter issue subtitle" />
                </AppFormControl>

                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <AppFormControl label="Volume" :error="errors.volume">
                        <AppInput name="volume" type="number" min="1" placeholder="e.g. 1" />
                    </AppFormControl>

                    <AppFormControl label="Issue Number" :error="errors.issue_number">
                        <AppInput name="issue_number" type="number" min="1" placeholder="e.g. 12" />
                    </AppFormControl>
                </div>

                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <AppFormControl label="Cover Date" :error="errors.cover_date">
                        <AppInput name="cover_date" type="date" />
                    </AppFormControl>
                ...
                <AppFormControl label="Editor" :error="errors.editor">
                    <AppInput name="editor" placeholder="Editor name" />
                </AppFormControl>

                <AppFormControl label="Description" :error="errors.description">
                    <AppTextarea name="description" rows="6" placeholder="Describe this issue..." />
                </AppFormControl>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <Button type="button" variant="outline" as-child>
                        <Link :href="index(magazine.id).url">
                            Cancel
                        </Link>
                    </Button>
                </div>
