...
<template>
    <div class="flex gap-4 py-5">
        ...
        <Heading title="Edit Research"
            description="Update the basic details of this research. Methodology, sources, questions, findings, surveys, reports, and other details are managed separately." />
    </div>

    <Form v-slot="{ errors, processing }" v-bind="update.form(props.research.id)" :options="{
        preserveScroll: true,
    }" class="space-y-4">
        <AppFormControl label="Title" required :error="errors.title">
            <AppInput name="title" placeholder="Enter research title" :default-value="research.title" />
        </AppFormControl>

        <AppFormControl label="Slug" required :error="errors.slug">
            <AppInput name="slug" placeholder="Enter research slug" :default-value="research.slug" />
        </AppFormControl>

        <AppFormControl label="Subtitle" :error="errors.subtitle">
            <AppInput name="subtitle" placeholder="Enter research subtitle" :default-value="research.subtitle ?? ''" />
        </AppFormControl>

        <AppFormControl label="Summary" :error="errors.summary">
            <AppTextarea name="summary" placeholder="Enter research summary" :default-value="research.summary ?? ''" />
        </AppFormControl>
        ...
        <AppFormControl label="Description" :error="errors.description">
            <AppTextarea name="description" placeholder="Enter research description"
                :default-value="research.description ?? ''" />
        </AppFormControl>
        ...
        <AppFormControl label="Research Type" required :error="errors.type">
            <AppSelect name="type" placeholder="Select research type" :options="typeOptions"
                :default-value="research.type.value" />
        </AppFormControl>
        ...
        <AppFormControl label="Research Status" required :error="errors.status">
            <AppSelect name="status" placeholder="Select research status" :options="statusOptions"
                :default-value="research.status.value" />
        </AppFormControl>

        <AppFormControl label="Published At" :error="errors.published_at">
            <AppInput name="published_at" type="datetime-local" :default-value="research.published_at
        ...
        <AppCheckbox name="featured" true-value="1" false-value="0" label="Feature this research"
            :default-value="research.featured ? '1' : '0'" />
