<script setup lang="ts">
    ...
    import { index as magazineIndex } from '@/routes/admin/magazine'
    import { create as categoryCreate, edit as categoryEdit, destroy as categoryDestroy } from '@/routes/admin/magazine/categories'
    import type { MagazineCategory } from '@/types'

    defineProps<{ categories: MagazineCategory[] }>()

    const toDelete = ref<MagazineCategory | null>(null)
    const deleting = ref(false)

    function confirmDelete() {
        if (!toDelete.value) return
        deleting.value = true
        router.delete(categoryDestroy(toDelete.value.id).url, {
            preserveScroll: true,
...
<template>
        <div class="max-w-4xl mx-auto px-4 py-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-xl font-bold text-gray-900 dark:text-white">Magazine Categories</h1>
                    <Link :href="magazineIndex().url" class="text-sm text-[#e0006c] hover:underline">&larr; Back to issues</Link>
                </div>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                        <tr v-for="cat in categories" :key="cat.id">
                            ...
                            <td class="px-5 py-3 text-gray-500 dark:text-gray-400">{{ cat.magazines_count ?? 0 }}</td>
                            <td class="px-5 py-3">
        ...
        <AppConfirmDialog :open="!!toDelete" title="Delete category?"
            :description="`Issues in “${toDelete?.name}” will be moved to Uncategorized.`" danger confirm-text="Delete"
