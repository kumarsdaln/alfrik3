...
import interviews, { show } from '@/routes/interviews'
...
const imageMedia = computed(() => {
    return (props.interview.media ?? []).find(
        media =>
...
const imageSrc = computed(() => {
    ...
    const src = imageMedia.value?.url
...
const formatConfig = computed(() => {
    switch (props.interview.interview_type.value) {
        case 'video':
            return {
    ...
    <article class="group w-full">
        <Link :href="show(interview.slug).url" :aria-label="interview.title" class="block">
            ...
            <div class="
                    ...
                    bg-neutral-100
                ">
                <img :src="imageSrc" :alt="imageMedia?.alt ??
                    interview.title
                ...
                <div class="absolute left-3 top-3">
                    <Badge variant="secondary">
                        <component :is="formatConfig.icon" :size="13" :stroke-width="1.8" :class="interview.interview_type.value === 'video'
                            ? 'fill-current'
                            : ''
                            " />
                ...
                <div v-if="
                    interview.interview_type.value !== 'written'
                " class="
                        absolute
                        bottom-3
                        right-3
                    ...
                    ">
                    <Play v-if="
                        interview.interview_type.value === 'video'
                    " :size="15" :stroke-width="1.8" class="ml-0.5 fill-current" />
                ...
                <div v-if="participants.length" class="
                    ...
                    <div class="flex shrink-0 -space-x-2">
                        <Avatar v-for="participant in visibleParticipants" :key="participant.id"
                            :name="participant.user?.name" :image="participant.user?.avatar" size="size-7"
